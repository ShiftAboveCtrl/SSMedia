# Subless TV Essential Mode Documentation

## Purpose

Essential mode is a streamlined Subless TV experience for users who want to install add-ons, find something, and play it without navigating the full customization surface.
...
1. `MainActivity` shows the Subless account QR sign-in flow if the first-launch auth prompt has not been completed.
...
> I just want Subless TV to work with the least setup possible.
...
| Trakt connection | Show as optional account integration or hide under Advanced | Show full | For Essential, prefer Subless account sync first. |
